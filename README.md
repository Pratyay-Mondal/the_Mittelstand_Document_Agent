...
4. **Fully Responsive Workspace**: A split-screen responsive layout with strict flex height constraints (`min-h-0`), sticky input controls, and custom markdown renderers for tables, code blocks, and source citations across all device screen sizes.
5. **Robust Local Vector Storage**: Auto-provisions vector collections with intelligent local disk persistence (`./qdrant_storage`) and singleton connection management to handle high concurrency.
6. **Source Citations**: AI responses include clickable citations tracing back to exact document chunks and page numbers.
7. **Dynamic i18n UI**: Instantly toggle the UI between English and German, built using `zustand` and Shadcn components.
...
|---|---|---|
| **Frontend** | Next.js 15, TypeScript, Tailwind | Core framework, responsive layout routing |
| **UI Library** | Shadcn/ui, Framer Motion, Radix UI | Accessible, enterprise-grade components & responsive primitives |
| **Backend** | Python, FastAPI, Pydantic | High-performance API server |
| **AI/LLM** | LangChain, Ollama (`gemma4:latest` / `llama3.1:8b`) | Local RAG orchestration & structured data extraction |
| **Vector DB** | Qdrant (Docker with Local Fallback) | Dense vector storage with auto-collection creation |
...
- Python 3.11+
- Node.js 22+
- [Ollama](https://ollama.com/) running locally with models: `gemma4:latest` (or `llama3.1:8b`) and `nomic-embed-text`
- Docker Engine *(Optional, but recommended for Qdrant)*

### 1. Vector Database
The application supports two modes for the Qdrant vector database:
- **Local Fallback (No Docker)**: If Docker is not running, the backend will automatically fall back to local persistent storage (`./qdrant_storage`) with concurrent thread safety and automated collection management.
