...
export function ChatPanel({ docId }: { docId: string }) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-card border-r border-border overflow-hidden">
        ...
        <div className="space-y-6">
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex w-full ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div 
                className={`max-w-[95%] md:max-w-[85%] min-w-0 rounded-2xl px-4 md:px-5 py-3 ${
                  msg.role === 'user' 
                    ? 'bg-primary text-primary-foreground rounded-tr-sm' 
                    : 'bg-muted border border-border rounded-tl-sm'
                }`}
              >
                <div className={`prose prose-sm dark:prose-invert max-w-none break-words min-w-0 ${msg.role === 'user' ? 'text-primary-foreground' : ''}`}>
                  <ReactMarkdown
                    components={{
                      table: ({ node, ...props }) => (
                      ...
                      pre: ({ node, ...props }) => (
                        <div className="overflow-x-auto my-2 rounded-lg bg-black/40 p-3 text-xs">
