...
export function ExtractionPanel({ docId }: { docId: string }) {
  ...
  if (loading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center h-full text-muted-foreground gap-4">
        ...
        <p>Extracting data natively via Gemma 4...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-background overflow-hidden">
      <div className="p-4 border-b border-border flex flex-wrap items-center justify-between gap-4 bg-card text-card-foreground flex-shrink-0">
        <div>
      ...
      <CardContent className="p-3">
        ...
        <div className="flex items-end gap-2">
           {isEditing ? (
             ...
             <div className="flex-1 flex items-center justify-between group cursor-text min-w-0" onClick={() => setIsEditing(true)}>
                <p className="text-sm font-medium leading-tight min-h-6 py-1 select-all break-words overflow-hidden w-full pr-2">{val || <span className="text-muted-foreground italic">Needs Human Input</span>}</p>
                <div className="flex items-center gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground">
